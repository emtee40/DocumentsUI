    ...
    public void setUp() throws Exception {
        ...
        try {
            if (!bots.notifications.isNotificationAccessEnabled(
                ...
                Context settings_context = context.createPackageContext(
                        SETTINGS_PACKAGE_NAME, Context.CONTEXT_RESTRICTED);
                int res_id = settings_context.getResources().getIdentifier(
                        "allow", "string", SETTINGS_PACKAGE_NAME);
                String allow_string = settings_context.getResources().getString(res_id);

                bots.notifications.setNotificationAccess(
                        getActivity(), ACCESS_APP_NAME, allow_string);
            }
        } catch (Exception e) {
            fail("Cannot set notification access. " + e.toString());
        }
        ...
        try {
            if (bots.notifications.isNotificationAccessEnabled(
                ...
                Context settings_context = context.createPackageContext(
                        SETTINGS_PACKAGE_NAME, Context.CONTEXT_RESTRICTED);
                int res_id = settings_context.getResources().getIdentifier(
                        "notification_listener_disable_warning_confirm",
                        "string", SETTINGS_PACKAGE_NAME);
                String trun_off_string = settings_context.getResources().getString(res_id);

                bots.notifications.setNotificationAccess(
                        getActivity(), ACCESS_APP_NAME, trun_off_string);
            }
        } catch (Exception e) {
            fail("Cannot set notification access. " + e.toString());
        }
